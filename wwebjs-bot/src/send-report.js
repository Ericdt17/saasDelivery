      ...
      client.on("ready", async () => {
        ...
        try {
          ...
          if (targetGroup) {
            ...
            console.log(`   Sending to configured recipient: ${config.REPORT_RECIPIENT}`);
            chat = await client.getChatById(chatId);
            ...
            console.error("   3. Use: npm run report:send -- --phone=612345678");
          ...
          try {
            ...
            const found = messages.find(m => m.fromMe && m.id?._serialized === sentMessage.id?._serialized);
            if (found) {
