...
async function testConnection() {
  ...
  if (config.DB_TYPE === "postgres") {
    console.log(`🔗 Connection: ${config.DATABASE_URL ? "Using DATABASE_URL" : `${config.PG_HOST}:${config.PG_PORT}/${config.PG_DATABASE}`}`);
  } else {
  ...
  try {
    ...
    if (config.DB_TYPE === "postgres") {
      ...
      console.log(`   PostgreSQL Version: ${result.version?.substring(0, 50)}...`);
      console.log(`   Current Time: ${result.current_time}`);
    } else {
      console.log("⏳ Testing SQLite connection...");
      const result = await adapter.query("SELECT datetime('now') as current_time, sqlite_version() as version");
      console.log("✅ Connection successful!");
      // SQLite returns single object for LIMIT 1 or single row queries
    ...
    const tableNames = config.DB_TYPE === "postgres" 
      ? tables.map(t => t.table_name)
      : tables.map(t => t.name);

    if (tableNames.includes("deliveries") && tableNames.includes("delivery_history")) {
      console.log("✅ Required tables exist:");
      console.log("   ✓ deliveries");
      console.log("   ✓ delivery_history");

      // Get record counts
      const deliveryCount = await adapter.query("SELECT COUNT(*) as count FROM deliveries");
      const historyCount = await adapter.query("SELECT COUNT(*) as count FROM delivery_history");
      
      let deliveryCnt, historyCnt;
      if (config.DB_TYPE === "postgres") {
        deliveryCnt = deliveryCount[0]?.count;
        historyCnt = historyCount[0]?.count;
      } else {
        // SQLite returns array for SELECT queries
        deliveryCnt = Array.isArray(deliveryCount) ? deliveryCount[0]?.count : deliveryCount?.count;
        historyCnt = Array.isArray(historyCount) ? historyCount[0]?.count : historyCount?.count;
      }
      ...
      console.log("⚠️  Some tables are missing. They will be created automatically.");
    }
