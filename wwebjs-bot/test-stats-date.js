...
async function testStatsDate(testDate) {
  ...
  try {
    ...
    console.log('  - total_collected:', stats.total_collected, typeof stats.total_collected);
    console.log('  - total_remaining:', stats.total_remaining, typeof stats.total_remaining);
    ...
    if (stats.total === 0) {
      ...
      if (allZero) {
        console.log('✅ Cohérence: total = 0, toutes les autres valeurs sont aussi 0');
      } else {
        console.error('❌ Incohérence: total = 0 mais certaines valeurs ne sont pas 0!');
        console.error('   Cela indique un problème dans la requête SQL ou le traitement des résultats');
      }
    } else {
    ...
    if (testDate === today) {
      ...
      if (stats.total === todayStats.total) {
        ...
        console.warn('⚠️  Les stats ne correspondent pas. Possible problème de timezone?');
