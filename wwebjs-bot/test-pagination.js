...
async function testPagination() {
  ...
  try {
    ...
    if (data1.pagination?.total > 10) {
      ...
      if (hasOverlap) {
        console.error('   ❌ PROBLÈME: Les pages contiennent des IDs en commun!');
      } else {
    ...
    if (data1.pagination?.totalPages > 1) {
      ...
      const pageLast = await fetch(`${API_URL}/api/v1/deliveries?page=${lastPage}&limit=10`);
    ...
    const pageFiltered = await fetch(`${API_URL}/api/v1/deliveries?page=1&limit=10&startDate=${today}&endDate=${today}`);
    ...
    if (error.code === 'ECONNREFUSED') {
      ...
      console.error('   Démarre le serveur avec: npm run dev (dans wwebjs-bot/)');
    }
    process.exit(1);
  }
}
