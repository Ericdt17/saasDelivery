...
const calculateWeeklyPerformance = (deliveries: any[]) => {
  ...
  const daysMap = new Map<string, { date: Date; dateStr: string; jour: string; livrees: number; echecs: number; pickups: number; expeditions: number }>();
  ...
  const chartData = useMemo(() => {
    ...
    const last7DaysDeliveries = deliveriesData.deliveries.filter((delivery) => {
      if (!delivery.date_creation) {
        console.log('[PerformanceChart] Delivery missing date_creation:', delivery.id);
        return false;
      }
      ...
      if (isNaN(deliveryDate.getTime())) {
        console.log('[PerformanceChart] Invalid date:', delivery.date_creation, delivery.id);
        return false;
      }
    ...
    <div className="stat-card">
      ...
      <div className="h-[300px]">
        {safeChartData && safeChartData.length > 0 ? (
          <ResponsiveContainer width="100%" height="100%" key={JSON.stringify(safeChartData)}>
            <BarChart data={safeChartData} margin={{ top: 10, right: 10, left: -10, bottom: 0 }}>
            ...
            <Bar dataKey="echecs" fill="hsl(var(--destructive))" radius={[4, 4, 0, 0]} />
            <Bar dataKey="pickups" fill="hsl(var(--info))" radius={[4, 4, 0, 0]} />
            <Bar dataKey="expeditions" fill="hsl(var(--expedition))" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ResponsiveContainer>
        ) : (
