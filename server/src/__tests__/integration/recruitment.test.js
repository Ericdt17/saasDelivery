...
/** Full valid multipart apply (all fields required). */
function validApplyMultipart(overrides = {}) {
  const {
    skipQuartier = false,
    skipTransport = false,
    skipAvailability = false,
    skipEmail = false,
    skipEducationLevel = false,
    skipFieldOfStudy = false,
    skipSchoolName = false,
    skipLanguages = false,
    skipCurrentlyEmployed = false,
    skipInOtherCompany = false,
    skipPhoto = false,
    skipCv = false,
  ...
  let req = request(app).post('/api/v1/recruitment/apply');
  req = req.field('job_offer_id', extraFields.job_offer_id ?? '1');
  req = req.field('full_name', extraFields.full_name ?? 'Jean Dupont');
  req = req.field('phone', phone);
  if (!skipEmail) req = req.field('email', extraFields.email ?? 'jean@example.com');
  if (!skipQuartier) req = req.field('quartier', extraFields.quartier ?? 'Makepe');
  if (!skipEducationLevel) {
    req = req.field('education_level', extraFields.education_level ?? 'licence');
  }
  if (!skipFieldOfStudy) {
    req = req.field('field_of_study', extraFields.field_of_study ?? 'Logistique');
  }
  if (!skipSchoolName) {
    req = req.field('school_name', extraFields.school_name ?? 'Université de Douala');
  }
  if (!skipLanguages) {
    req = req.field('languages', extraFields.languages ?? 'francais,anglais');
  }
  if (!skipCurrentlyEmployed) {
    req = req.field(
      'currently_employed',
      extraFields.currently_employed ?? 'non'
    );
  }
  if (!skipInOtherCompany) {
    req = req.field('in_other_company', extraFields.in_other_company ?? 'non');
  }
  if (!skipTransport) req = req.field('transport', extraFields.transport ?? 'scooter');
  if (!skipAvailability) {
    req = req.field('availability', extraFields.availability ?? 'plein');
  }
  req = req.field('answers', answers);
  if (!skipPhoto) {
    req = req.attach('photo', Buffer.from('fakejpeg'), {
      filename: 'face.jpg',
      contentType: 'image/jpeg',
    });
  }
  if (!skipCv) {
    req = req.attach('cv', Buffer.from('%PDF-1.4'), {
      filename: 'cv.pdf',
      contentType: 'application/pdf',
    });
  }
  if (!skipCoverLetter) {
    req = req.attach('cover_letter', Buffer.from('%PDF-1.4'), {
      filename: 'lm.pdf',
      contentType: 'application/pdf',
    });
  }
...
function expectInvalidField(res, fieldName) {
  expect(res.status).toBe(400);
  expect(res.body.success).toBe(false);
  expect(res.body.invalidFields?.some((f) => f.field === fieldName)).toBe(true);
}
...
describe('POST /api/v1/recruitment/apply', () => {
  ...
  it('returns 400 when quartier is missing', async () => {
    const res = await validApplyMultipart({ skipQuartier: true });
    expectInvalidField(res, 'quartier');
  });

  it('returns 400 when email is missing', async () => {
    const res = await validApplyMultipart({ skipEmail: true });
    expectInvalidField(res, 'email');
  });

  it('returns 400 when education_level is missing', async () => {
    const res = await validApplyMultipart({ skipEducationLevel: true });
    expectInvalidField(res, 'education_level');
  });

  it('returns 400 when field_of_study is missing', async () => {
    const res = await validApplyMultipart({ skipFieldOfStudy: true });
    expectInvalidField(res, 'field_of_study');
  });

  it('returns 400 when school_name is missing', async () => {
    const res = await validApplyMultipart({ skipSchoolName: true });
    expectInvalidField(res, 'school_name');
  });

  it('returns 400 when languages is missing', async () => {
    const res = await validApplyMultipart({ skipLanguages: true });
    expectInvalidField(res, 'languages');
  });

  it('returns 400 when languages contains invalid value', async () => {
    const res = await validApplyMultipart({ extraFields: { languages: 'allemand' } });
    expect(res.status).toBe(400);
    expect(res.body.invalidFields?.some((f) => f.field === 'languages')).toBe(true);
  });

  it('returns 400 when currently_employed is missing', async () => {
    const res = await validApplyMultipart({ skipCurrentlyEmployed: true });
    expectInvalidField(res, 'currently_employed');
  });

  it('returns 400 when in_other_company is missing', async () => {
    const res = await validApplyMultipart({ skipInOtherCompany: true });
    expectInvalidField(res, 'in_other_company');
  });

  it('returns 400 when transport is missing', async () => {
    const res = await validApplyMultipart({ skipTransport: true });
    expectInvalidField(res, 'transport');
  });

  it('returns 400 when availability is missing', async () => {
    const res = await validApplyMultipart({ skipAvailability: true });
    expectInvalidField(res, 'availability');
  });

  it('returns 400 when photo is missing', async () => {
    const res = await validApplyMultipart({ skipPhoto: true });
    expectInvalidField(res, 'photo');
  });

  it('returns 400 when cv is missing', async () => {
    const res = await validApplyMultipart({ skipCv: true });
    expectInvalidField(res, 'cv');
  });

  it('returns 400 when cover_letter is missing', async () => {
    const res = await validApplyMultipart({ skipCoverLetter: true });
    expectInvalidField(res, 'cover_letter');
  });

  it('returns 400 when job is closed or not found', async () => {
    mockRecruitmentGetOpenJobOfferById.mockResolvedValueOnce(null);
    const res = await validApplyMultipart();
    expect(res.status).toBe(400);
  ...
  it('returns 400 when answers JSON is malformed', async () => {
    ...
    const res = await validApplyMultipart({ answers: '{not json}' });
    expect(res.status).toBe(400);
    expect(res.body.error).toMatch(/valid JSON/i);
  });

  it('returns 400 when job has questions but answers are missing or empty', async () => {
    mockRecruitmentGetOpenJobOfferById.mockResolvedValueOnce(jobFixture);
    mockRecruitmentListQuestionsForJobOffer.mockResolvedValueOnce([questionFixture]);
    const res = await validApplyMultipart({ answers: '[]' });
    expect(res.status).toBe(400);
  ...
  it('returns 400 when answers reference a question not in this job', async () => {
    ...
    const answers = JSON.stringify([
      { question_id: 10, answer_text: 'Oui' },
  ...
  it('creates application and returns 201 with id when all fields are provided', async () => {
    mockRecruitmentGetOpenJobOfferById.mockResolvedValueOnce(jobFixture);
    mockRecruitmentListQuestionsForJobOffer.mockResolvedValueOnce([questionFixture]);
  ...
  it('accepts required photo field (multipart)', async () => {
    mockRecruitmentGetOpenJobOfferById.mockResolvedValueOnce(jobFixture);
    mockRecruitmentListQuestionsForJobOffer.mockResolvedValueOnce([]);
    mockRecruitmentCreateApplicationWithAnswers.mockResolvedValueOnce({ id: 100 });
    const res = await validApplyMultipart();
    ...
    expect(mockRecruitmentCreateApplicationWithAnswers).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        cover_letter_original_name: 'lm.pdf',
      })
    );
  });
  ...
  it('accepts profile_photo field as alias for photo (multipart)', async () => {
    ...
    const res = await request(app)
      ...
      .field('email', 'jean@example.com')
      .field('quartier', 'Makepe')
      .field('education_level', 'licence')
      .field('field_of_study', 'Logistique')
      .field('school_name', 'Université de Douala')
      .field('languages', 'francais,anglais')
      .field('currently_employed', 'non')
      .field('in_other_company', 'non')
      .field('transport', 'scooter')
      .field('availability', 'plein')
      .field('answers', '[]')
      ...
      .attach('cover_letter', Buffer.from('%PDF-1.4'), {
        filename: 'lm.pdf',
        contentType: 'application/pdf',
      });
    expect(res.status).toBe(201);
  ...
  it('accepts photo and cv in the same multipart request', async () => {
    ...
    mockRecruitmentListQuestionsForJobOffer.mockResolvedValueOnce([]);
    mockRecruitmentCreateApplicationWithAnswers.mockResolvedValueOnce({ id: 100 });
    const res = await validApplyMultipart({ phone: '699000022' });
    expect(res.status).toBe(201);
    expect(mockRecruitmentCreateApplicationWithAnswers).toHaveBeenCalledWith(
      expect.objectContaining({
        job_offer_id: 1,
  ...
  it('accepts job_id and neighborhood as aliases for job_offer_id and quartier', async () => {
    ...
    const res = await request(app)
      ...
      .field('education_level', 'bac')
      .field('field_of_study', 'Commerce')
      .field('school_name', 'Lycée de Bonamoussadi')
      .field('languages', 'francais')
      .field('currently_employed', 'oui')
      .field('in_other_company', 'oui')
      .field('transport', 'scooter')
      .field('availability', 'plein')
      .field('answers', '[]')
