...
Creates a candidature with **all fields required**: identity, formation, quartier, transport, availability, photo, CV, cover letter, and an answer for **every** custom question on the offer.

**Doc détaillée** : [server/docs/recrutement-formulaire-candidature.md](server/docs/recrutement-formulaire-candidature.md)

**Body:** `multipart/form-data` only.

**UX (landing)** : présenter l’upload **photo avant** le CV dans le formulaire. Le protocole HTTP n’impose pas l’ordre des parties du multipart.

| Champ multipart | Alias accepté | Type | Obligatoire | Contraintes | Exemple valide |
|-----------------|---------------|------|-------------|-------------|----------------|
| `job_offer_id` | `job_id` | string ou number (coercé) | oui | Entier Postgres **> 0** | `"2"` |
| `full_name` | — | string | oui | non vide | `"Jean Dupont"` |
| `phone` | — | string | oui | 1–32 caractères (trim côté serveur) | `"699000001"` |
| `email` | — | string | oui | email valide, max 255 | `"jean@example.com"` |
| `quartier` | `neighborhood` | string | oui | non vide après trim | `"Bonamoussadi"` |
| `education_level` | — | string | oui | **`bac`**, **`licence`**, **`master`**, **`doctorat`** | `licence` |
| `field_of_study` | `study_field` | string | oui | non vide, max 255 | `"Logistique"` |
| `school_name` | `school` | string | oui | non vide, max 255 | `"Université de Douala"` |
| `languages` | — | string | oui | **`francais`**, **`anglais`** — au moins une ; `"francais,anglais"` ou JSON array | `francais,anglais` |
| `currently_employed` | — | string | oui | **`oui`**, **`non`** | `non` |
| `in_other_company` | — | string | oui | **`oui`**, **`non`** | `non` |
| `transport` | — | string | oui | **`scooter`**, **`velo`**, **`voiture`**, **`apied`** | `scooter` |
| `availability` | — | string | oui | **`plein`**, **`partiel`**, **`weekend`** | `plein` |
| `answers` | — | string | oui si l’offre a des questions | Chaîne JSON : tableau d’objets **`{ "question_id": number \| string, "answer_text": string }`**. Une entrée **non vide** par question de l’offre. | `"[{\"question_id\":10,\"answer_text\":\"Oui\"}]"` |
| `photo` | `profile_photo`, `picture` | fichier | oui | JPEG, PNG ou WebP ; max **10 MB**. Si Cloudinary non configuré → **503**. | fichier image |
| `cv` | `file`, `resume` | fichier | oui | PDF uniquement ; max **10 MB** | fichier `.pdf` |
| `cover_letter` | `motivation_letter`, `letter` | fichier | oui | PDF uniquement ; max **10 MB** | fichier `.pdf` |

**Réponses d’erreur 400 (validation corps de formulaire)**

- `error`: `"Validation failed"` ou message dédié (`Answer required for question N`, etc.)
- `invalidFields`: liste `{ field, messages[] }` (noms **canoniques** : `job_offer_id`, `quartier`, `photo`, `cv`, `cover_letter`, etc.)
- `details`: objet Zod aplati (`fieldErrors`, `formErrors`) pour le détail champs texte/enum

Autres 400 : JSON `answers` invalide, `question_id` dupliqué, réponse QCM hors options, question inconnue, offre fermée, etc.

**409** : candidature déjà existante pour le même `(job_offer_id, phone)`.

**201** : `{ "success": true, "data": { "id": <application_id> } }`.

**Front public** : pages `/recrutement` (liste) et `/recrutement/:jobId/postuler` (formulaire complet).
